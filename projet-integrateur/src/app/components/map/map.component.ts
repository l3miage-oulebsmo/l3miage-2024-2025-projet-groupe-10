...
import { Icon, icon, LatLng, latLng, marker, Polyline, polyline, tileLayer } from 'leaflet';
...
export class MapComponent {
  ...
  constructor() {
    // La carte sera initialisée dans onMapReady
  }
  ...
  calculateRoute(): void {
    ...
    if (this.locations.length > 1) {
      ...
      const openRouteApiUrl = "/api/v2/directions/driving-car";
      ...
      .then(response => {
        ...
        if (response.data.routes && response.data.routes.length > 0) {
          ...
          const routeCoordinates = response.data.routes[0].geometry.coordinates.map((coord: number[]) => [coord[1], coord[0]]);
